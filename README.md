...
RedHawk 用自签 CA 做 HTTPS 中间人，为每个访问的域名动态签发证书。**只有把这张 CA 装进系统「受信任的根证书颁发机构」，浏览器才会接受它**，否则访问 HTTPS 会报 `NET::ERR_CERT_AUTHORITY_INVALID`。

| 方法 | 操作 | 权限 |
|---|---|---|
| 一键（推荐） | 抓包代理 →「⚙ 安装 CA 证书」→ UAC 弹窗点「是」 | 需 UAC |
| 脚本（⚠ 需管理员） | 右键 `data\安装CA证书.bat` → **以管理员身份运行**（写入「受信任的根证书」需提权，非管理员会提示后退出）；或执行 `fix_ca.ps1`（自动清理旧 CA + 安装新 CA） | 前者需管理员；`fix_ca.ps1` 无需 |
| 手动 | 「🔑 下载 CA 证书」→ 右键 `.crt` → 安装证书 → 本地计算机 → 受信任的根证书颁发机构 | — |

> ⚠️ 装完**必须彻底重启浏览器**（关掉所有窗口再开）。
>
> 若之前装过旧 CA 仍报 `NET::ERR_CERT_AUTHORITY_INVALID`，多为「信任根里的 CA」与「代理当前签名的 CA」同名不同钥匙。运行 `fix_ca.ps1` 自动清理旧同名 CA 并重装新 CA，再重启浏览器即可。
